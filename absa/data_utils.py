...
def xlsx2seg(data_path = 'datasets/dutch_data.xlsx'):
    '''
    将 xlsx 文件转换为 .seg 的格式
    '''
    aspects = ['service', 'product', 'price', 'effect', 'logistics', 'package']

    df = pd.read_excel(data_path)
    output_file = open(data_path+'.seg', 'w')
    for text, label in zip(df['text'], df['Label']):
        print(text)
        label = str(label).rstrip().replace(' ', ',').replace(':', ',').split(',')
        # 一次只替换一个 $T$，将多个 Aspect 拆开
        for i in range(0, len(label), 2):
            print(label)
            # 还要考虑没有 Aspect 的情况, 填入一个 null
            if label[i] == 'nan':
...
def build_embedding_matrix(word2idx, embed_dim, dat_fname):
    ...
    if os.path.exists(dat_fname):
        ...
        # 要求 嵌入维度 必须为 300
        fname = 'pre_models/word2vec.txt' if embed_dim != 300 else 'pre_models/word2vec.txt'

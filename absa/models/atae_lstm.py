...
class ATAE_LSTM(nn.Module):
    ...
    def forward(self, inputs):
        # 有一些unsqueeze(增维)和squeeze(降维)是因为LSTM的输入(也是 RNN 的输入)为(num_steps, batch_size, embed_size) 其中包括了 batch_size
        # 而中间传播的过程中不包含 batch_size（因为模型的输入不考虑批量大小），而输出依然要包含 batch_size，所以要先降维再增维
